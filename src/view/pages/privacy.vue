<template>
  <div class="info_content">
    ...
    <p v-show="lang=='ZH'">
				...
				<br>
				如您对本用户隐私政策或其他相关事宜有疑问、建议、投诉，请通过我们设立的个人信息保护的专职部门进行联系。联系方式是：luckylami88@gmail.com（邮箱）。我们将在收到您的问题后30天内予以答复。
	</p>
    <p v-show="lang=='EN'">
				...
				<br>
				If you have any questions, suggestions, or complaints about this user privacy policy or other related matters, please contact the full-time department for personal information protection established by us. The contact method is: luckylami88@gmail.com (email). We will reply within 30 days after receiving your question.
	</p>
    <div class="lang delete" v-show="lang=='EN'">
        <button @click="detleteMsg" v-if="!loading_true&&!loading">Delete Personal Data</button>
        <p v-show="loading">Deleting</p>
        <p v-show="loading_true">Successfully deleted personal data, thank you very much for your attention!</p>
    </div>
    <div class="lang delete" v-show="lang=='ZH'">
        <button @click="detleteMsg" v-if="!loading_true&&!loading">删除个人信息</button>
        <p v-show="loading">正在删除中</p>
        <p v-show="loading_true">删除个人信息成功，非常感谢您的关注！</p>
    </div>
  </div>
</template>
